...
|---|---|---|
| [daylight-port](daylight-port/SKILL.md) | web app → the right container (PWA / thin WebView-shell APK / native) + every web↔Android collision fixed (touch, selection, keyboard, offline, back button, downloads). Includes a complete no-Gradle APK shell template. | solid first pass; shell template awaits first real build |
| [daylight-design](daylight-design/SKILL.md) | redesign for the medium: color→grayscale mapping that actually reads on LivePaper, 10.5″ 4:3 tablet layout, touch/stylus/voice-first, calm. Preserves the app's own soul. | solid first pass; **awaiting Anjan's design system** in `references/design-system.md` |
